...
func main() {
	...
	go func() {
		defer collectWg.Done()
		startTime := time.Now()
		resp, err := callGeminiAPI("pro-2.5", prompt, geminiKey)
		duration := time.Since(startTime)
		resultsChan <- ModelResponse{
			Model:    "Gemini",
	...
	go func() {
		defer collectWg.Done()
		startTime := time.Now()
		resp, err := callGeminiAPI("flash-2.5", prompt, geminiKey)
		duration := time.Since(startTime)
		resultsChan <- ModelResponse{
			Model:    "Gemini",
...
func generateSummary(responses []ModelResponse, startTime time.Time) {
	...
	combinedPrompt := "Combine the following outputs from different AI models. Analyze at all of them, combine them, and make sure you don't omit any of each one's individual best parts\n"
